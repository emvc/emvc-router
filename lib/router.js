...
Router.prototype.resource = function(name, options, fn) {
  ...
  var actions = [ 'show', 'new', 'create', 'edit', 'update', 'destroy' ]
  ...
  var self = this
    , showActionIndex = actions.indexOf('show')
    , getResourceHelper = utils.once(function () {
        return helper;
      }, null);

  // Moving `show` action to the beginning of the actions list, in front of `create`, `update` and `destroy`.
  // It is important to generate it's helper first, because it's route has optional "format" parameter.
  if (showActionIndex > 0) {
    actions.splice(showActionIndex, 1);
    actions.unshift('show');
  }
  ...
  actions.forEach(function(action) {
    switch (action) {
      case 'new':     self._route('get' , path + '/new.:format?' , controller, 'new'     , utils.functionize('new', helper));
        break;
      case 'create':  self._route('post', path                   , controller, 'create'  , getResourceHelper());
        break;
      case 'show':    self._route('get' , path + '.:format?'     , controller, 'show'    , getResourceHelper());
        break;
      case 'edit':    self._route('get' , path + '/edit.:format?', controller, 'edit'    , utils.functionize('edit', helper));
        break;
      case 'update':  self._route('put' , path                   , controller, 'update'  , getResourceHelper());
        break;
      case 'destroy': self._route('delete' , path                   , controller, 'destroy' , getResourceHelper());
        break;
    }
  });
...
Router.prototype.resources = function(name, options, fn) {
  ...
  var actions = [ 'index', 'show', 'new', 'create', 'edit', 'update', 'destroy' ]
    , ns = this._ns[this._ns.length - 1]
  ...
  var self = this
    ...
    , getCollectionHelper = utils.once(function () {
        return collectionHelper;
      }, null);

  // Moving 'index' and 'show' actions to the beginning of the actions list, in front of other actions.
  // It is important to generate helpers for them first, because they have optional "format" parameters.
  actions = actions.sort(function (a1, a2) {
    if (a1 === 'index' || a1 === 'show') {
      return -1;
    } else if (a2 === 'index' || a2 === 'show') {
      return 1;
    } else {
      return 0;
    }
  ...
  actions.forEach(function(action) {
    switch (action) {
      case 'index':   self._route('get' , path + '.:format?'                   , controller, 'index'   , getCollectionHelper());
        break;
      case 'new':     self._route('get' , path + '/new.:format?'               , controller, 'new'     , utils.functionize('new', helper));
        break;
      case 'create':  self._route('post', path                                 , controller, 'create'  , getCollectionHelper());
        break;
      case 'show':    self._route('get' , path + '/' + param + '.:format?'     , controller, 'show'    , getResourceHelper());
        break;
      case 'edit':    self._route('get' , path + '/' + param + '/edit.:format?', controller, 'edit'    , utils.functionize('edit', helper));
        break;
      case 'update':  self._route('put' , path + '/' + param                   , controller, 'update'  , getResourceHelper());
        break;
      case 'destroy': self._route('delete' , path + '/' + param                   , controller, 'destroy' , getResourceHelper());
        break;
    }
  });
