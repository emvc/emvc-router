...
/**
 * Generates function, which will call provided function only once and return it's result.
 * All subsequent calls it will return this cached result, or the value of `futherResult`, if provided.
 *
 * @param {Function} fn
 * @param [futherResult]
 * @returns {Function}
 */
exports.once = function (fn, futherResult) {
  var executed = false
    , futherResultPassed = (arguments.length > 1);
  ...
  return function () {
    ...
    if (executed) {
      ...
      if (!futherResultPassed) {
        futherResult = fnResult;
      }
      return fnResult;
    }
  }
